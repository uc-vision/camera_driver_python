...
from camera_driver.config import ImageSettings, ToneMapper, Transform
from camera_driver.image.encoding import BayerPattern, bayer_pattern, encoding_bits
...
class FrameProcessor(Dispatcher):
  ...
  def __init__(self, cameras:Dict[str, CameraInfo], settings:ImageSettings, logger:Logger, device:torch.device):
    ...
    self.processor = TaichiQueue.run_sync(self.init_processor, cameras)
    self.queue.start()
  ...
  @beartype
  def init_processor(self, cameras:Dict[str, CameraInfo]):
  ...
  def check_image(self, k, image:torch.Tensor):
  ...
  def process_worker(self, camera_images:Dict[str, CameraImage]):
    images = [self.check_image(k, image.image_data) 
              for k, image in camera_images.items()]

    images = TaichiQueue.run_sync(self.process_images, images)
